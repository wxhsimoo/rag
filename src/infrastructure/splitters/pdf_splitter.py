...
class PdfSplitter(InfraDocumentSplitter):
    """PDF 文本切分器

    PDF 已在加载阶段转为纯文本，此切分器对文本进行分段。
    ...
    def _init_langchain_splitter(self) -> None:
        if LANGCHAIN_AVAILABLE:
            try:
                ...
                if self.logger:
                    self.logger.info("Initialized Langchain RecursiveCharacterTextSplitter for PdfSplitter")
            except Exception as e:
                if self.logger:
                    self.logger.warning(f"Failed to initialize Langchain PDF splitter: {str(e)}")
                self.text_splitter = None
    ...
    def _split_with_langchain(self, text: str, metadata: Dict[str, Any]) -> List[InfraDocumentChunk]:
        try:
            ...
            for i, d in enumerate(docs):
                ...
                chunk.set_metadata('pdf_method', 'langchain_recursive_character')
                chunks.append(chunk)
            if self.logger:
                self.logger.debug(f"Langchain split PDF text into {len(chunks)} chunks")
            return chunks
        except Exception as e:
            if self.logger:
                self.logger.error(f"Error in Langchain PDF splitting: {str(e)}")
            return self._split_with_fallback(text, metadata)
        ...
        if len(paragraphs) > 1:
            ...
            for paragraph in paragraphs:
                ...
                if len(candidate) <= chunk_size:
                    ...
                    if current_chunk.strip():
                        ...
                        chunk.set_metadata('pdf_method', 'fallback_paragraph')
                        chunks.append(chunk)
            ...
            while start < len(text):
                ...
                if content.strip():
                    ...
                    chunk.set_metadata('pdf_method', 'fallback_character')
                    chunks.append(chunk)
                start = end - overlap
                if start <= 0:
                    start = end
        if self.logger:
            self.logger.debug(f"Fallback split PDF text into {len(chunks)} chunks")
        return chunks
