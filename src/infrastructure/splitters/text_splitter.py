...
class TextSplitter(InfraDocumentSplitter):
    """纯文本切分器，适用于 .txt / .text 文件
    ...
    def _init_langchain_splitter(self) -> None:
        if LANGCHAIN_AVAILABLE:
            try:
                ...
                if self.logger:
                    self.logger.info("Initialized Langchain RecursiveCharacterTextSplitter for TextSplitter")
            except Exception as e:
                if self.logger:
                    self.logger.warning(f"Failed to initialize Langchain Text splitter: {str(e)}")
                self.text_splitter = None
    ...
    def _split_with_langchain(self, text: str, metadata: Dict[str, Any]) -> List[InfraDocumentChunk]:
        try:
            ...
            for i, d in enumerate(docs):
                ...
                chunk.set_metadata('text_method', 'langchain_recursive_character')
                chunks.append(chunk)
            if self.logger:
                self.logger.debug(f"Langchain split Text into {len(chunks)} chunks")
            return chunks
        except Exception as e:
            if self.logger:
                self.logger.error(f"Error in Langchain Text splitting: {str(e)}")
            return self._split_with_fallback(text, metadata)
    ...
    def _split_with_fallback(self, text: str, metadata: Dict[str, Any]) -> List[InfraDocumentChunk]:
        ...
        if self.config.separators:
            ...
            for sep in self.config.separators:
                ...
                base = temp
            paragraphs = [p for p in base if p.strip()]
        else:
        ...
        if len(paragraphs) > 1:
            ...
            for paragraph in paragraphs:
                ...
                if len(candidate) <= chunk_size:
                    ...
                    if current_chunk.strip():
                        ...
                        chunk.set_metadata('text_method', 'fallback_paragraph')
                        chunks.append(chunk)
            ...
            while start < len(text):
                ...
                if content.strip():
                    ...
                    chunk.set_metadata('text_method', 'fallback_character')
                    chunks.append(chunk)
                start = end - overlap
                if start <= 0:
                    start = end
        if self.logger:
            self.logger.debug(f"Fallback split Text into {len(chunks)} chunks")
        return chunks
